...
export function calculateRandomPosition(containerRect: DOMRect): { x: number; y: number } {
  return {
...
export function keepWithinBounds(
  ...
  return {
    x: Math.max(0, Math.min(position.x, containerRect.width - TASK_CARD.WIDTH)),
    y: Math.max(0, Math.min(position.y, containerRect.height - TASK_CARD.HEIGHT)),
  };
}
...
export function createTaskRectangle(position: { x: number; y: number }): Rectangle {
  return {
    left: position.x,
...
export function calculateOverlap(rect1: Rectangle, rect2: Rectangle): { x: number; y: number } {
  return {
...
export function calculatePushDirection(
  ...
  if (overlap.x < overlap.y) {
    // Push horizontally
    if (currentRect.right - otherRect.left < otherRect.right - currentRect.left) {
    ...
    // Push vertically
    if (currentRect.bottom - otherRect.top < otherRect.bottom - currentRect.top) {
